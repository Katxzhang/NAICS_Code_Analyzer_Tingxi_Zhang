...
# Read the case data
d_6digit <- read.csv("data/6_digits_des.csv", encoding = "UTF-8")
d_4digit <- read.csv("data/4_digits_des.csv", encoding = "UTF-8")
d_2digit <- read.csv("data/2_digits_des.csv", encoding = "UTF-8")
code_6digit <- read.csv("data/6_digits_code.csv", header=FALSE,encoding = "UTF-8")
code_4digit <- read.csv("data/4_digits_code.csv", header=FALSE,encoding = "UTF-8")
code_2digit <- read.csv("data/2_digits_code.csv", header=FALSE,encoding = "UTF-8")
...
function(input, output, session){
  ...
  file1<- reactive({
    ...
    code_bs_similarity6 <- code_bs_similarity6 %>%
      head(3)
  ...
  file2<- reactive({
    ...
    code_bs_similarity4 = code_bs_similarity4 %>%
      head(2)
  ...
  file3 <- reactive({
    ...
    business_desc <- data.frame(Industry = "Business Name & Description",Description = business_desc)
    # Combine source code and text file
    d_2digit_bs <- rbind (business_desc,d_2digit)
    # Change column name of both combined documents
    names(d_2digit_bs)[1] = "doc_id"
    names(d_2digit_bs)[2] = "text"
    
    # Transfer the data into corpus 
    doc_corpus2 = VCorpus(DataframeSource(d_2digit_bs))
    # Apply data pre-processing to each corpus
    corpus_cleaned2 <- corpus_preprocessing(doc_corpus2)
    # Document term matrix
    # Apply TF-IDF Weighting 
    tfidf_2 <- DocumentTermMatrix(corpus_cleaned2,control = list(weighting = weightTfIdf))
    tfidf_m2 = as.matrix(tfidf_2)
    # Calculate Cosine Similarity for each digit 
    tfidf_cos_sim2 = round(cos_sim(tfidf_m2), digits = 2)
    # Create columns for similarity_score and corresponding code 
    d_2digit_bs["naics_2digit"] = code_2digit
    d_2digit_bs["similarity_score"] = tfidf_cos_sim2[1:ncol(tfidf_cos_sim2)]
    # Sort the data frame by similarity score
    code_bs_similarity2 = d_2digit_bs[order(-d_2digit_bs$similarity_score),]
    #Remove the first line of business description 
    code_bs_similarity2 = code_bs_similarity2[-1,]
    
    code_bs_similarity2 = code_bs_similarity2 %>%
      head(1)
    
    #Remove rowname 
    rownames(code_bs_similarity2) <- NULL
    #Download the output still the full code 
    output$download_code_bs_similarity_2digit <- downloadHandler(
      filename = function(){
        paste("industry_sim_2_digit-",Sys.Date(),".csv",sep = "")
      },
      content = function(file){
        write.csv(code_bs_similarity2,file)
      })
    return(code_bs_similarity2)
    
  })
    
    output$table_6_digit <- DT::renderDataTable({
      DT::datatable(file1(),options = list(dom='t'))
    })
    output$table_4_digit <- DT::renderDataTable({
      DT::datatable(file2(),options = list(dom='t'))
    })
    
    output$table_2_digit <- DT::renderDataTable({
      DT::datatable(file3(),options = list(dom='t'))
    })
}
